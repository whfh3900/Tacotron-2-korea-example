...
def length_up():
	output_path = './wavs_output/wavs/'
	for i in os.listdir(output_path):
    		if i.endswith('.wav'):
        		data = wave.open(output_path+i)
        		buffer = data.readframes(data.getnframes())
        		amptitude = (np.frombuffer(buffer, dtype="int16")).astype(np.float64)   
        
        		amptitude = np.append(amptitude,[0]*100)   
        
        		clean_wave = wave.Wave_write(output_path+i)
        		clean_wave.setparams(data.getparams())
        		clean_wave.writeframes(array.array('h', amptitude.astype(np.int16)).tostring() )
        		clean_wave.close()
        		data.close()
...
def main():
	...
	parser.add_argument('--length_up', default=False, help='합성완료후 음성이 끊기는 것을 방지')
