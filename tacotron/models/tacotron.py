...
class Tacotron():
	...
	def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None, targets_lengths=None, gta=False,
		...
		for i in range(hp.tacotron_num_gpus):
			...
			print('initialisation done {}'.format(gpus[i]))
		...
		print('Initialized Tacotron model. Dimensions (? = dynamic shape): ')
		log('Initialized Tacotron model. Dimensions (? = dynamic shape): ')
		print('  Train mode:               {}'.format(is_training))
		log('  Train mode:               {}'.format(is_training))
		print('  Eval mode:                {}'.format(is_evaluating))
		log('  Eval mode:                {}'.format(is_evaluating))
		print('  GTA mode:                 {}'.format(gta))
		log('  GTA mode:                 {}'.format(gta))
		print('  Synthesis mode:           {}'.format(not (is_training or is_evaluating)))
		...
		for i in range(hp.tacotron_num_gpus):
			...
			log('  device:                   {}'.format(i))
			print('  embedding:                {}'.format(tower_embedded_inputs[i].shape))
			log('  embedding:                {}'.format(tower_embedded_inputs[i].shape))
			print('  enc conv out:             {}'.format(tower_enc_conv_output_shape[i]))
			log('  enc conv out:             {}'.format(tower_enc_conv_output_shape[i]))
			print('  encoder out:              {}'.format(tower_encoder_outputs[i].shape))
			log('  encoder out:              {}'.format(tower_encoder_outputs[i].shape))
			print('  decoder out:              {}'.format(self.tower_decoder_output[i].shape))
			log('  decoder out:              {}'.format(self.tower_decoder_output[i].shape))
			print('  residual out:             {}'.format(tower_residual[i].shape))
			log('  residual out:             {}'.format(tower_residual[i].shape))
			print('  projected residual out:   {}'.format(tower_projected_residual[i].shape))
			log('  projected residual out:   {}'.format(tower_projected_residual[i].shape))
			print('  mel out:                  {}'.format(self.tower_mel_outputs[i].shape))
			log('  mel out:                  {}'.format(self.tower_mel_outputs[i].shape))
			if post_condition:
				print('  linear out:               {}'.format(self.tower_linear_outputs[i].shape))
				log('  linear out:               {}'.format(self.tower_linear_outputs[i].shape))
			print('  <stop_token> out:         {}'.format(self.tower_stop_token_prediction[i].shape))
			log('  <stop_token> out:         {}'.format(self.tower_stop_token_prediction[i].shape))

		#1_000_000 is causing syntax problems for some people?! Python please :)
		print('  Tacotron Parameters       {:.3f} Million.'.format(np.sum([np.prod(v.get_shape().as_list()) for v in self.all_vars]) / 1000000))
