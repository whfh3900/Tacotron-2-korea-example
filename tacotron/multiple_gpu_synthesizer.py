...
class Synthesizer:
	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames):
		...
		#print('len(texts) = {}\nbasenames = {}\nlen(basenames) = {}'.format(len(texts),basenames,len(basenames)))
		...
		if self.gta or not hparams.predict_linear:
			...
			mels1 = np.clip(mels[0], T2_output_range[0], T2_output_range[1])	
			mels2 = np.clip(mels[1], T2_output_range[0], T2_output_range[1])	
			mels = [mels1]+[mels2]
			...
			texts = [[mel for gpu_mels in texts for mel in gpu_mels]]
			#print('len(mels) == len(linears) == len(texts) {} {} {}'.format(len(mels),len(linears),len(texts)))
	...
	def _get_output_lengths(self, stop_tokens):
		stop_tokens = [[mel for gpu_mels in stop_tokens for mel in gpu_mels]]
		#print('stop_tokens = {}\nstop_tokens[0] = {}'.format(len(stop_tokens),len(stop_tokens[0])))
