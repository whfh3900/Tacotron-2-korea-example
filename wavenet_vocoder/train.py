...
def eval_step(sess, global_step, model, plot_dir, wav_dir, summary_writer, hparams, model_name):
	...
	print('Eval loss for global step {}: {:.3f}'.format(global_step, loss))
...
def train(log_dir, args, hparams, input_path):
	...
	print('Wavenet training set to a maximum of {} steps'.format(args.wavenet_train_steps))
	...
	with tf.Session(config=config) as sess:
		try:
			...
			if args.restore:
				...
				try:
					...
					if (checkpoint_state and checkpoint_state.model_checkpoint_path):
						...
						#print('No model to load at {}'.format(save_dir), slack=True)
			...
			if run_init:
				...
				print('Data dependent initialization done. Starting training!')
			...
			while not coord.should_stop() and step < args.wavenet_train_steps:
				...
				#print(message, end='\r', slack=(step % args.checkpoint_interval == 0))
				...
				if hparams.gin_channels > 0 and (step % args.embedding_interval == 0 or step == args.wavenet_train_steps or step == 1):
					...
					print('WaveNet Speaker embeddings have been updated on tensorboard!')
					log('WaveNet Speaker embeddings have been updated on tensorboard!')

			#print('Wavenet training complete after {} global steps'.format(args.wavenet_train_steps), slack=True)
