...
hparams = tf.contrib.training.HParams(
	...
	num_freq = 1103, # (= n_fft / 2 + 1) only used when adding linear spectrograms post processing network
	...
	#Mel spectrogram
	n_fft = 2205, #Extra window size is filled with 0 paddings to match this parameter
	hop_size = 551, #For 22050Hz, 275 ~= 12.5 ms (0.0125 * sample_rate)
	win_size = 2205, #For 22050Hz, 1100 ~= 50 ms (If None, win_size = n_fft) (0.05 * sample_rate)
...
#out_channels = 2, #This should be equal to quantize channels when input type is 'mulaw-quantize' else: num_distributions * 3 (prob, mean, log_scale).
	...
	upsample_scales = [19, 29], #prod(upsample_scales) should be equal to hop_size
...
#tacotron_synthesis_batch_size = 8*16, #DO NOT MAKE THIS BIGGER THAN 1 IF YOU DIDN'T TRAIN TACOTRON WITH "mask_encoder=True"!!
...
#tacotron_test_batches = 40, #number of test batches.
	...
	'지금부터 최대한 길게 말해보겠습니다. 마이크 폼페이오 미국 국무장관이 상원 외교위 청문회 발언을 통해 북한의 비핵화.',
	'안녕하세요. 지금부터 한류 에이아이 리서치를 소개하겠습니다.',
	'영어로 hanryu a i research 라고 씁니다.',
	#'심리학은 인간의 행동과 심리과정을 과학적으로 연구하는 경험 과목이다.',
	#'모두가 시험에 통과했음을 발표하게 되어 기쁘게 생각합니다.',
	#'교수와 학생은 갑을 관계가 아니라 사제지간 입니다.',
	#'부모가 저지르는 큰 실수 중 하나는 자기 아이를 다른 집 아이와 비교하는 것이다.',
	'정부는 이 지역을 관광지로 개발할 계획이라고 발표했다.',
	'그 호텔은 호화로운 시설들을 갖추고 있다.', 
	'그녀는 수영복으로 갈아입은 다음 밖으로 나갔다.',
	'이런 것들이 우리가 갖고 있는 부정적 커뮤니케이션의 대표적인 속성들이 아니었던가요?',
	'안녕하세요. 내일 점심 식사 이후에 잠깐 뵐수 있을까요?',
	'전방 100미터 앞에서 좌회전 하세요.',
	'전방 2 100미터 앞에서 우회전 하세요.',
	'와! 이런 것도 가능하구나! 어? 잠깐만.',
